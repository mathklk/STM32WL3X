...
#define HAL_MRSUBG_SET_CHFLT_M(__M__)                 MODIFY_REG_FIELD(MR_SUBG_GLOB_DYNAMIC->MOD1_CONFIG, MR_SUBG_GLOB_DYNAMIC_MOD1_CONFIG_CHFLT_M, __M__);
#define HAL_MRSUBG_SET_CHFLT_E(__E__)                 MODIFY_REG_FIELD(MR_SUBG_GLOB_DYNAMIC->MOD1_CONFIG, MR_SUBG_GLOB_DYNAMIC_MOD1_CONFIG_CHFLT_E, __E__);
...
typedef struct {
	uint32_t numberOfBuffersUsed;
	uint32_t iqSampleCount;
	uint8_t buffer;
} BufferInfo;

volatile static inline BufferInfo HAL_MRSUBG_GET_BUFFER_INFO() {
	BufferInfo bi = {
		HAL_MRSUBG_GET_NUMBER_OF_DATABUFFERS(),
		HAL_MRSUBG_GET_DATABUFFER_COUNT() / 4,
		HAL_MRSUBG_GET_CURRENT_DATABUFFER()
	};
	return bi;
}
